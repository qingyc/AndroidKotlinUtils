...
object ScanHelper {
    ...
    fun scanFile(ctx: Context, filepath: String) {
        ...
        MediaScannerConnection.scanFile(ctx, paths, null) { path, uri ->
            QLogger.i("path:$path uri:${uri.toString()}")
            //path:/storage/emulated/0/VACO/pictures/20190830181941.png uri:content://media/external/file/300908
    ...
    fun refreshFileNow(filepath: String) {
        try {//下次打开相册才会刷新
            ...
            if (Build.VERSION.SDK_INT >= 24) {
                // Provider分享
                val uriForFile = FileProvider.getUriForFile(ApplicationProxy.getApp().applicationContext,
    ...
    fun refreshImageDelete(imagePath: String) {
        ApplicationProxy.getApp().contentResolver.delete(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, MediaStore.Images.Media.DATA + "=?", arrayOf(imagePath));//删除系统缩略图
    }
