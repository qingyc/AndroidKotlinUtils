...
object FileSaveUtil {
    ...
    fun saveWithRequestStoragePermission(
        activity: RxAppCompatActivity,
        bitmap: Bitmap?,
        path: String = FileOpUtils.getNoMediaPictureDirectory(),
        fileSuffix: String = FileOpUtils.PNG_FILE_SUFFIX,
        action: (success: Boolean, outputFilePath: String, error: Throwable?) -> Any
    ) {
        if (bitmap == null) {
                    ...
                    when {
                        ...
                        //永久被拒
                            ...
                            val dialog = CommonDialog.newInstance(
                                ResUtils.getString(R.string.tip_storage_open),
                                ResUtils.getString(R.string.allow)
                            )
    ...
    fun bitmap2File(
        ...
        //文件保存的时间
        val date =
            SimpleDateFormat("yyyyMMddHHmmss", Locale.US).format(Date(System.currentTimeMillis()))
